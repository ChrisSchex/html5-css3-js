...
function setupPuzzle() {
   ...
   /* Set the initial color of each cell to white */
   for (var i = 0; i < allCells.length; i++) {
      allCells[i].style.backgroundColor = "white";
      allCells[i].style.color = "black";
      allCells[i].style.borderRadius = 0;

      /* Change the color with each mousedown */
...
function findErrors() {
   ...
   setTimeout(
      function() {      
         /* Change all cells with red font back to white */
         for (var i = 0; i < allCells.length; i++) {
            if (allCells[i].style.color === "red") {allCells[i].style.color = "white";}
            }         
      } , 1000);
}
